 ...
 */
function loadConfig(path = "./config.json", jsonManualConfig = "{}") {
    ...
    fs.readFile(path, "utf8", (err, data) => {
      if (err) {
        ...
        try {
          ...
          console.error(
            `Ошибка парсинга JSON файла конфигурации: ${error.message}`
          );
        }
      }
      try {
        const manualConfig = JSON.parse(jsonManualConfig);
        Object.assign(config, manualConfig);
      } catch (error) {
        console.error(`Ошибка парсинга JSON объекта: ${error.message}`);
      }
      resolve(config);
    });
  });
